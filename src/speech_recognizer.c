...
static FILE *fdwav = NULL;//文件打开接口

static int open_stored_file(const char * name)//函数 打开文件
{
	fdwav = fopen(name, "wb+");
	if(fdwav == NULL) {
...
static int loopwrite_to_file(char *data, size_t length)//函数 循环写入
{
...
static void safe_close_file()//函数 安全关闭文件
{
	if(fdwav) {
		fclose(fdwav);
		fdwav = NULL;
...
static void end_sr_on_error(struct speech_rec *sr, int errcode)//函数 结束
{
