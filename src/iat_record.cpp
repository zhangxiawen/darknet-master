...
static int deter_result(char *string)
{
	char *keystr1 = "我的面前有什么。";
	char *keystr2 = "测试。";
	char *keystr3 = "停止服务。";
	char *keystr4 = "本地输入。";
	if (*string == *keystr1)  return 1;
	if (*string == *keystr2)  return 2;
	if (*string == *keystr3)  return 3;
	if (*string == *keystr4) return 4;
}
 ...
 */
int iat_record()   /*int argc, char* argv[]*/
{
	int			ret						=	MSP_SUCCESS;
	int			upload_on				=	1; //是否上传用户词表
	//const char* login_params			=	"appid = 5aabb0f6, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
	int aud_src = 0;
	int flag = 0;
